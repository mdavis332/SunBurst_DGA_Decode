...
def main():
    ...
    parser.add_argument('-a', '--action', choices=('encode', 'decode'), default='decode')
    ...
    if action == 'decode':
        ...
        final = []
        for line in in_file:
            # run encode many times to get possible permutations b/c math is hard
            for i in range(500):
                temp = Encode(line.strip())
                if temp not in final:
                    final.append(temp)
        for result in final:
            out_file.write('xxxxxxxxxxxxxxxx{0}.appsync-api.domain2.avsvmcloud[.]com\n'.format(result))
